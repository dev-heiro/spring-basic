...
public class BookingController {
    ...
    /*
        자동으로 addAttribute 될때 클래스명으로 등록이되는데, (앞글자만 소문자로 변경되서)

        @Valid 를 쓸때는 그 다음 인자를 반드시 BindingResult 로 설정해야됨.
     */
    @RequestMapping("/proceed")
    public String bookingProceedHandle( HttpSession session,
                                        @ModelAttribute @Valid TableBooking booking,BindingResult result,
                                        Model model
                                         ) {
